...
# ==============================================================================
# STREAMLIT APPLICATION UI (with Back and Skip Navigation)
# ==============================================================================
def main():
    st.set_page_config(page_title="Keedam Quiz", page_icon="✈️", layout="centered")
    st.title("✈️ Keedam's Quiz Generator")
    ...
    # This dictionary holds all our persistent variables.
    if 'state' not in st.session_state:
        st.session_state.state = 'initial'
        st.session_state.questions = []
        st.session_state.current_question = 0
        # Using a dictionary for answers is better for non-linear navigation
        st.session_state.user_answers = {} 
    ...
    if st.session_state.state == 'finished':
        # Calculate score from the answers dictionary
        ...
        with st.expander("Review Your Answers"):
             for i, q in enumerate(st.session_state.questions):
                # Use .get() to safely handle skipped questions, providing a default message
    ...
    if st.session_state.state in ['quiz_started', 'show_feedback']:
        ...
        def jump_to_question():
            ...
            # Extract the number from "Question X" to get the new index
            new_index = int(selected_q_text.split(" ")[1]) - 1
            st.session_state.current_question = new_index
            # Always go back to the answering state when jumping to a new question
            st.session_state.state = 'quiz_started'

        st.selectbox(
            "Skip to question:",
            options=[f"Question {i+1}" for i in range(len(st.session_state.questions))],
            index=st.session_state.current_question,
            on_change=jump_to_question,
            key='question_jumper' # A key is needed for on_change to work
        )
        st.markdown("---")
        ...
        if st.session_state.state == 'quiz_started':
            with st.form(key=f"form_{q_index}"):
                # Set the default index based on any previous answer for this question
                previous_answer_index = 0
                if q_index in st.session_state.user_answers:
                    try:
                        previous_answer_index = valid_options.index(st.session_state.user_answers[q_index])
                    except ValueError:
                        previous_answer_index = 0
            ...
            # --- NEW BACK/NEXT BUTTONS IN COLUMNS ---
            # col1, col2, col3 = st.columns([1, 1, 1])

            # with col1:
            #     # Disable the back button if we are on the first question
